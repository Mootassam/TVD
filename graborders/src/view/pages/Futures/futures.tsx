...
function Futures() {
  ...
  // Prevent navigation during active demo trade
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      // Check if a demo trade modal is open (tradeDirection indicates modal is open)
      if (isDemoAccount && tradeDirection) {
        e.preventDefault();
        e.returnValue = '';
      }
    };
  ...
  }, [isDemoAccount, tradeDirection]);

  // Determine if user is on demo account
  useEffect(() => {
    setIsDemoAccount(currentUser?.accountType === 'demo');
  }, [currentUser]);
  ...
  const connectWebSocket = useCallback(() => {
    ...
    ws.onopen = () => {
      ...
      // Subscribe to the current symbol (using ref to always have the latest)
       subscribeToSymbol(getTvSymbol(selectedCoinRef.current));
    };
  ...
  useEffect(() => {
     subscribeToSymbol(getTvSymbol(selectedCoin));
  }, [selectedCoin, subscribeToSymbol]);

  // Derive current price, high, low, and change percent
  useEffect(() => {
     const market = markets.find((m) => m.symbol === getTvSymbol(selectedCoin));
    ...
    if (initialPriceRef.current[getTvSymbol(selectedCoin)] === undefined) {
      initialPriceRef.current[getTvSymbol(selectedCoin)] = midPrice;
      setPriceChangePercent(0);
    } else {
      const initial = initialPriceRef.current[getTvSymbol(selectedCoin)];
      const change = ((midPrice - initial) / initial) * 100;
      setPriceChangePercent(change);
    }

    if (!highRef.current[getTvSymbol(selectedCoin)] || midPrice > highRef.current[getTvSymbol(selectedCoin)]) {
      highRef.current[getTvSymbol(selectedCoin)] = midPrice;
    }
    if (!lowRef.current[getTvSymbol(selectedCoin)] || midPrice < lowRef.current[getTvSymbol(selectedCoin)]) {
      lowRef.current[getTvSymbol(selectedCoin)] = midPrice;
    }
  ...
  const high = highRef.current[getTvSymbol(selectedCoin)] ?? (currentPrice ?? 0);
  const low = lowRef.current[getTvSymbol(selectedCoin)] ?? (currentPrice ?? 0);
